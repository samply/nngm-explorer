...
import { alias as aliasMap, cqltemplate, criterionMap } from '$lib/cqlquery-mappings';
...
export const translateAstToCql = (
	...
	} else if (additionalCriteria.substring(additionalCriteria.length - 9) == 'intersect') {
	...
	const specimenMeasure = localMeasures.find((element) => element.key == 'specimen');
				...
				if (Array.isArray(criterion.value)) {
					...
					if (values.length === 1) {
						...
						values.forEach((value: string) => {
							expression +=
								'(' +
								substituteCQLExpression(criterion.key, myCriterion.alias, myCQL, value) +
								') or\n';
						});
